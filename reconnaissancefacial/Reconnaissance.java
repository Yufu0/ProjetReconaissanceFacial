...
public class Reconnaissance extends Application {
	...
	public TreeItem arborescence(){
		...
		for (File file : dirApprentissage) {
			fichier = file.listFiles();
			TreeItem<String> rootSA = new TreeItem<>(file.getName());
			for (File fil : fichier) {
				rootSA.getChildren().add(new TreeItem<String>(fil.getName()));
			}
			rootApprentissage.getChildren().addAll(
					rootSA
			);
		}
		for (File file : dirTest) {
			fichier = file.listFiles();
			TreeItem<String> rootST = new TreeItem<>(file.getName());
			for (File fil : fichier) {
				rootST.getChildren().add(new TreeItem<String>(fil.getName()));
			}
			rootTest.getChildren().addAll(
					rootST
			);
		}
